...
void main() {
  group('groupedBy', () {
    group('sync', () {
      test('should group into (key, items) records in first-seen key order',
          () {
        final groups =
            groupedBy((String w) => w.length, ['ab', 'cd', 'e', 'fg']);
      ...
      test('should match groupBy exactly', () {
        ...
        expect(
            asGroups.map((g) => g.key).toList(), equals(asMap.keys.toList()));
    ...
    group('async', () {
      test('should group into records in first-seen key order', () async {
        final groups = await groupedByAsync(
            (int n) async => n.isEven, toAsync([1, 2, 3, 4]));
      ...
      test('should be able to be used in the pipeline', () async {
        final groups =
            await fx(['ab', 'c', 'de']).toAsync().groupedBy((w) => w.length);
        expect(groups.map((g) => g.key), equals([2, 1]));
