  ...
  group('sortByDesc', () {
    group('sync', () {
      ...
      test('should sort double keys descending (unboxed path)', () {
        expect(
          sortByDesc((double d) => d, [0.5, 2.5, 1.5]),
          equals([2.5, 1.5, 0.5]),
        );
      });
      ...
      test('should sort by an extracted key', () {
        expect(
          sortByDesc((String s) => s.length, ['bb', 'a', 'cccc']),
          equals(['cccc', 'bb', 'a']),
        );
      });
      ...
      test('should be able to be used in the pipeline', () {
        final top = fx([
          ...
          ('b', 9),
          ('c', 5),
        ]).sortByDesc((p) => p.$2).take(2).map((p) => p.$1).toList();
