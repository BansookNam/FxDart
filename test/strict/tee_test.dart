...
void main() {
  group('tee', () {
    ...
    test('steps see every element in order', () {
      final left = <int>[];
      final right = <int>[];
      tee([3, 1, 2], (seed: 0, step: (int a, int x) => (left..add(x)).length), (
        seed: 0,
        step: (int a, int x) => (right..add(x)).length,
      ));
    ...
    test('a throwing step propagates', () {
      expect(
        () => tee([1, 2], (seed: 0, step: (int a, int x) => a + x), (
          seed: 0,
          step: (int a, int x) => throw StateError('step'),
        )),
        throwsStateError,
      );
    });
