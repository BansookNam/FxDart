...
void main() {
  group('fused stages (map/filter/takeWhile)', () {
    test('sync stages produce the layered result and effect order', () async {
      final log = <String>[];
      final out = await toListAsync(mapAsync((int a) {
        log.add('m$a');
        return a * 10;
      },
          filterAsync((int a) {
    ...
    test('async callbacks inside fused stages', () async {
      final out = await toListAsync(mapAsync((int a) => delayed(a + 100),
          filterAsync((int a) => delayed(a > 1), toAsync([1, 2, 3]))));
      expect(out, equals([102, 103]));
    });

    test('takeWhile stage ends the whole fused chain (sync and async)',
        () async {
      expect(
          await toListAsync(mapAsync((int a) => a * 2,
    ...
    test('a sync callback throw rejects the pull', () async {
      await expectLater(
          toListAsync(mapAsync<int, int>((a) => throw StateError('boom'),
              toAsync([1, 2]))),
          throwsStateError);
    });

    test('filter-everything-out and empty sources drain to empty', () async {
      expect(await toListAsync(filterAsync((int a) => false, toAsync([1, 2]))),
          equals(<int>[]));
      expect(await toListAsync(mapAsync((int a) => a, toAsync(<int>[]))),
          equals(<int>[]));
    });
    ...
    test('marked first pull then unmarked pulls (legacy fallback mix)',
        () async {
  ...
  group('flatMap fast iterator', () {
    test('async source and async inner-iterable futures', () async {
      final out = await toListAsync(flatMapAsync(
    ...
    test('empty inner iterables are skipped', () async {
      final out = await toListAsync(flatMapAsync(
          (int a) => a % 2 == 0 ? [a] : <int>[], toAsync([1, 2, 3, 4])));
      expect(out, equals([2, 4]));
    });

    test('under concurrent(n)', () async {
      final out = await fxAsync(toAsync([1, 2, 3]))
          .map((a) => delayed(a))
          .flatMap((a) => [a, a])
          .concurrent(2)
          .toList();
  ...
  group('scan fast iterator', () {
    test('future seed and async accumulator', () async {
      final out = await toListAsync(scanAsync(
          (int acc, int a) => delayed(acc + a), delayed(10), toAsync([1, 2])));
  ...
  group('stream bridge and subscription drive', () {
    test('fromStream + fused stages collects via the drive', () async {
      final out = await toListAsync(mapAsync((String s) => s.toUpperCase(),
          filterAsync((String s) => s.startsWith('w'),
    ...
    test('drive with async stages pauses per element, order kept', () async {
      final out = await toListAsync(mapAsync(
          (int a) => delayed(a * 2),
          filterAsync((int a) => delayed(a != 2),
    ...
    test('drive stops at a failing takeWhile (sync and async)', () async {
      expect(
          await toListAsync(takeWhileAsync(
              (int a) => a < 3, fromStream(Stream.fromIterable([1, 2, 3, 4])))),
          equals([1, 2]));
      expect(
          await toListAsync(takeWhileAsync((int a) => delayed(a < 3),
    ...
    test('foldAsync drives stream-sourced chains (sync and async fold)',
        () async {
      expect(
          await foldAsync(
              0, (int acc, int a) => acc + a, fromStream(Stream.fromIterable([1, 2, 3]))),
          equals(6));
      expect(
          await foldAsync(0, (int acc, int a) => delayed(acc + a),
              fromStream(Stream.fromIterable([1, 2, 3]))),
          equals(6));
    });
    ...
    test('bridge error answers the waiting pull and ends the iteration',
        () async {
      final controller = StreamController<int>();
  ...
  group('using / timeout fast paths', () {
    test('usingAsync releases after a fast-pull drain', () async {
      var released = false;
      final out = await toListAsync(usingAsync(
          () => 'r',
    ...
    test('usingAsync releases when a fused inner throws synchronously',
        () async {
      var released = false;
      await expectLater(
          toListAsync(usingAsync(
              () => 'r',
              (r) => mapAsync<int, int>(
                  (a) => throw StateError('inner boom'), toAsync([1])),
              (r) => released = true)),
          throwsStateError);
      expect(released, isTrue);
    });

    test('timeoutAsync skips the timer for synchronously answered pulls',
        () async {
      // A sync source cannot stall: even a zero-ish limit never fires.
      final out = await toListAsync(timeoutAsync(
    ...
    test('timeoutAsync still fires on stalled asynchronous pulls', () async {
      await expectLater(
          toListAsync(timeoutAsync(
              const Duration(milliseconds: 5),
              mapAsync(
                  (int a) => Future.delayed(
  ...
  group('classic terminal loops (non-fast iterators)', () {
    test('fold/reduce/each over a concurrent chain use the classic loop',
        () async {
      final chain = concurrentAsync(
          2, mapAsync((int a) => delayed(a), toAsync([1, 2, 3, 4])));
      expect(await foldAsync(0, (int acc, int a) => acc + a, chain), equals(10));
      final chain2 = concurrentAsync(
          2, mapAsync((int a) => delayed(a), toAsync([1, 2, 3, 4])));
      expect(await reduceAsync((int acc, int a) => acc + a, chain2), equals(10));
      final chain3 = concurrentAsync(
          2, mapAsync((int a) => delayed(a), toAsync([1, 2])));
      final seen = <int>[];
  ...
  group('remaining machinery branches', () {
    test('nextOr after a marked pull delegates to the fallback', () async {
      Future<void> viaFallback(
          FxAsyncIterable<int> chain, List<int> expected) async {
      ...
      await viaFallback(mapAsync((int a) => a + 1, toAsync([1, 2, 3])), [2, 3, 4]);
      await viaFallback(
          scanAsync((int acc, int a) => acc + a, 0, toAsync([1, 2])), [0, 1, 3]);
      await viaFallback(
          flatMapAsync((int a) => [a, -a], toAsync([1, 2])), [1, -1, 2, -2]);
    });
    ...
    test('bridge error with several waiters: first errors, rest are done',
        () async {
      final controller = StreamController<int>();
      final it = fromStream(controller.stream).iterator;
      final f1 = it.next();
    ...
    test('toStream over a non-fast iterator uses the classic loop', () async {
      final out = await concurrentAsync(
              2, mapAsync((int a) => delayed(a * 2), toAsync([1, 2, 3])))
          .toStream()
          .toList();
    ...
    test('sync using releases when use() itself throws', () {
      ...
      final it = using<String, int>(
          () => 'r', (r) => throw StateError('use boom'), (r) => released = true)
          .iterator;
      expect(it.moveNext, throwsStateError);
      expect(released, isTrue);
    });

    test('scan legacy under concurrent: future seed and async accumulator',
        () async {
      final it = scanAsync((int acc, int a) => delayed(acc + a), delayed(10),
              toAsync([1, 2]))
          .iterator;
      final got = <int>[(await it.next(Concurrent.of(2))).value];
      while (true) {
    ...
    test('scan manual unmarked pulls take the gated public path', () async {
      final it = scanAsync((int acc, int a) => acc + a, 0, toAsync([1, 2]))
          .iterator;
    ...
    test('flatMap over an always-async source (stream bridge)', () async {
      final out = await toListAsync(flatMapAsync(
          (int a) => [a, a * 10], fromStream(Stream.fromIterable([1, 2]))));
      expect(out, equals([1, 10, 2, 20]));
    });

    test('flatMap legacy under concurrent with async inner iterables',
        () async {
      final out = await fxAsync(toAsync([1, 2]))
          .flatMap((a) => [a, a])
          .concurrent(2)
          .toList();
      expect(out, equals([1, 1, 2, 2]));
      final it = flatMapAsync((int a) => delayed([a, -a]), toAsync([1, 2]))
          .iterator;
      final got = <int>[(await it.next(Concurrent.of(2))).value];
      while (true) {
        final r = await it.next(Concurrent.of(2));
    ...
    test('takeWhile legacy keeps answering done after the end under load',
        () async {
      final out = await fxAsync(toAsync([1, 2, 3, 4, 5, 6]))
          .map((a) => delayed(a))
          .takeWhile((a) => a < 3)
    ...
    test('takeWhile legacy answers done on a pull issued after the end',
        () async {
      // Pull past the end on the marked (legacy) path: the `end` short
      // circuit answers without touching the upstream.
      final it = takeWhileAsync((int a) => a < 2, toAsync([1, 2, 3])).iterator;
