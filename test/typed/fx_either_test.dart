  ...
  group('sequenceEither', () {
    group('sync', () {
      test('collects all Rights', () {
        expect(
            sequenceEither(
                    <Either<String, int>>[Right(1), Right(2), Right(3)])
                .getOrNull(),
            [1, 2, 3]);
        expect(
            fx(<Either<String, int>>[Right(1), Right(2)])
                .sequence()
                .getOrNull(),
            [1, 2]);
      });
    ...
    group('async', () {
      test('collects all Rights', () async {
        final result =
            await fxAsync(toAsync(<Either<String, int>>[Right(1), Right(2)]))
                .sequence();
    ...
    group('sync', () {
      test('top-level: collects every failure', () {
        final result = mapOrAccumulate<String, int, int>(
            (r, n) => n.isEven ? n * 10 : r.raise('odd $n'), [1, 2, 3]);
      ...
      test('chain: returns all results when nothing fails', () {
        expect(
            fx([1, 2])
                .mapOrAccumulate<String, int>((r, n) => n * 10)
                .getOrNull(),
            [10, 20]);
      });

      test('chain: a branch can contribute multiple errors', () {
        final result = fx([1, 2]).mapOrAccumulate<String, int>((r, n) =>
