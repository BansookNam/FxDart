...
void main() {
  ...
  Future<ProcessResult> run(List<String> args) => Process.run(
    ...
    ['run', 'bin/install_skills.dart', ...args],
  );
  ...
  test('installs named agents into project directories, deduplicating shared '
    ...
    expect(
      File('${tmp.path}/.claude/skills/fxdart-pipelines/SKILL.md').existsSync(),
      isTrue,
    );
    expect(
    ...
    expect(
      File('${tmp.path}/.agents/skills/fxdart-pipelines/SKILL.md').existsSync(),
      isTrue,
    );
  ...
  test('auto-detects agents from marker directories', () async {
    ...
    expect(
      File(
        '${tmp.path}/.opencode/skills/fxdart-pipelines/SKILL.md',
      ).existsSync(),
      isTrue,
    );
  ...
  test(
    'fails with guidance when nothing is detected and no agent is named',
    () async {
      final result = await run(['--project-root', tmp.path]);
  ...
  test('--global installs under --home, including pi\'s '
    ...
    expect(
      File(
        '${tmp.path}/.pi/agent/skills/fxdart-pipelines/SKILL.md',
      ).existsSync(),
      isTrue,
    );
    expect(
      File(
        '${tmp.path}/.config/devin/skills/fxdart-pipelines/SKILL.md',
      ).existsSync(),
      isTrue,
    );
  });
  ...
  test('--remove deletes only fxdart skills, --list reports status', () async {
    ...
    expect(
      File('${tmp.path}/.claude/skills/other-skill/SKILL.md').existsSync(),
      isTrue,
    );
  });
  ...
  test('reinstall replaces stale files', () async {
    await run(['--project-root', tmp.path, 'claude']);
    final stale = File('${tmp.path}/.claude/skills/fxdart-pipelines/stale.txt')
