...
void main() {
  group('windowed', () {
    group('sync', () {
      ...
      test('should stop when the source ends inside the gap between windows',
          () {
        expect(
      ...
      test('chunk should equal windowed with step = size and partials', () {
        for (final n in [1, 2, 3, 4, 7]) {
          expect(toList(chunk(n, range(1, 12))),
    ...
    group('async', () {
      test('should slide like the sync form', () async {
        ...
        expect(await toListAsync(windowedAsync(3, asyncEmpty<int>())),
            equals([]));
      });
      ...
      test('should reject non-positive size or step', () {
        ...
        expect(() => windowedAsync(2, toAsync([1, 2]), step: 0),
            throwsArgumentError);
      });
