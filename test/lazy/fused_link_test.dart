...
void main() {
  group('async scan inside a fused run', () {
    ...
    test('a Future seed dropped by a later filter still starts the run',
        () async {
      final result = await fxAsync(toAsync([1, 2, 3]))
          .scan((int acc, int a) => acc + a, Future.value(100))
          .filter((a) => a.isOdd)
    ...
    test('an asynchronous accumulator resumes a run that can also drop',
        () async {
  ...
  group('async takeWhile inside a fused run', () {
    test('ends the run when the predicate fails asynchronously', () async {
      final result = await fxAsync(toAsync([1, 2, 3, 4, 1]))
          .takeWhile((a) async => a < 3)
          .toList();
    ...
    test('ends the run after a filter, over an asynchronous source', () async {
      final result = await fxAsync(asyncSource([2, 4, 5, 6]))
          .filter((a) => a.isEven)
          .takeWhile((a) async => a < 6)
          .toList();
    ...
    test('a failing takeWhile stops pulling the source', () async {
      var pulled = 0;
      final result = await fxAsync(toAsync([1, 2, 3, 4, 5]))
          .peek((_) => pulled++)
          .takeWhile((a) async => a < 3)
          .toList();
  ...
  group('fused drive error paths', () {
    test('a throwing source fails the terminal', () async {
      ...
      expect(fxAsync(fromStream(boom())).map((a) => a).toList(),
          throwsStateError);
    });
    ...
    test('a stage throwing synchronously fails the terminal', () async {
      expect(
          fxAsync(asyncSource([1, 2, 3]))
              .map((a) => a == 2 ? throw StateError('stage') : a)
              .toList(),
          throwsStateError);
    });
    ...
    test('a throwing scan accumulator fails the terminal', () async {
      expect(
          fxAsync(asyncSource([1, 2, 3]))
              .scan((int acc, int a) async => throw StateError('acc'), 0)
              .toList(),
          throwsStateError);
    });
    ...
    test('a throwing filter predicate fails the terminal', () async {
      expect(
          fxAsync(asyncSource([1, 2, 3]))
              .filter((a) async => throw StateError('pred'))
              .toList(),
          throwsStateError);
    });
  });
  ...
  group('the pull path (a run wrapped by take)', () {
    ...
    test('runs a synchronous accumulator in a run that can also drop',
        () async {
      final result = await fxAsync(toAsync([1, 2, 3, 4]))
          .scan((int acc, int a) => acc + a, 0)
          .filter((a) => a.isEven)
    ...
    test('carries a Future seed through an asynchronous later stage',
        () async {
    ...
    test('ends a map-only run when the asynchronous source runs out',
        () async {
      final result = await fxAsync(asyncSource([1, 2]))
          .map((a) => a * 2)
          .take(10)
          .toList();
    ...
    test('drops and maps asynchronously over an asynchronous source',
        () async {
    ...
    test('runs an asynchronous accumulator in a run that can also drop',
        () async {
      final result = await fxAsync(asyncSource([1, 2, 3, 4]))
          .scan((int acc, int a) async => acc + a, 0)
          .filter((a) => a.isEven)
  ...
  group('fused drive synchronous-throw paths', () {
    test('a synchronous throw from the upstream run fails the terminal',
        () async {
      expect(
          fxAsync(toAsync([1, 2]))
    ...
    test('a synchronous throw while emitting the seed fails the terminal',
        () async {
      expect(
          fxAsync(toAsync([1, 2]))
              .scan((int acc, int a) => acc + a, 0)
  ...
  group('the compiled chain is equivalent to the unfused layering', () {
    test('a Concurrent marker falls back and agrees element for element',
        () async {
      final fused = await fxAsync(toAsync([1, 2, 3, 4, 5, 6]))
    ...
    test('each terminal sees exactly what toList sees', () async {
      FxAsync<int> chain() => fxAsync(toAsync([1, 2, 3, 4]))
          .scan((int acc, int a) async => acc + a, 0)
          .filter((a) => a.isEven);
      ...
      final viaFold =
          await chain().fold(<int>[], (List<int> acc, int a) => acc..add(a));
