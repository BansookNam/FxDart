  ...
  group('filterWithIndex', () {
    group('sync', () {
      test('keeps the elements the predicate holds for', () {
        expect(toList(filterWithIndex((a, i) => i.isEven, ['a', 'b', 'c'])),
            equals(['a', 'c']));
      });

      test('the index counts the input — dropped elements still advance it',
          () {
        final seen = <(String, int)>[];
        toList(filterWithIndex((String a, int i) {
          seen.add((a, i));
          return false;
        }, ['a', 'b', 'c']));
      ...
      test('is empty for an empty source', () {
        expect(toList(filterWithIndex((a, i) => true, <int>[])),
            equals(<int>[]));
      });
      ...
      test('combines the value and the index', () {
        final res = fx([10, 20, 30, 40])
            .filterWithIndex((a, i) => i.isOdd && a > 20)
            .toList();
        expect(res, equals([40]));
      });

      test('is available as an fx chain method', () {
        expect(fx(range(0, 6)).filterWithIndex((a, i) => i % 3 == 0).toList(),
            equals([0, 3]));
      });
    });
    ...
    group('async', () {
      ...
      test('the index counts the input', () async {
        final seen = <(String, int)>[];
        await toListAsync(filterWithIndexAsync((String a, int i) {
          seen.add((a, i));
          return false;
        }, toAsync(['a', 'b', 'c'])));
        expect(seen, equals([('a', 0), ('b', 1), ('c', 2)]));
      ...
      test('restarts the index on every iteration', () async {
        final it = filterWithIndexAsync((a, i) => i < 2, toAsync(['a', 'b', 'c']));
      ...
      test('is available as an fxAsync chain method', () async {
        final res = await fxAsync(toAsync(range(0, 6)))
            .filterWithIndex((a, i) => i % 3 == 0)
            .toList();
