  ...
  group('concat', () {
    group('sync', () {
      test("should be concatenated given two 'Iterable'", () {
        expect(
            toArray(concat([1, 2, 3], [4, 5, 6])), equals([1, 2, 3, 4, 5, 6]));
      });

      test('should be able to be used in the pipeline', () {
        final res = fx([1, 2, 3]).concat([4, 5, 6]).toArray();
    ...
    group('async', () {
      test("should be concatenated given two 'AsyncIterable'", () async {
        ...
        final b = mapAsync(
            (int x) => delay(const Duration(milliseconds: 50), x),
            toAsync([3, 4]));
        final res = await toArrayAsync(concatAsync(a, b));
      ...
      test("should be concatenated given two 'AsyncIterable' concurrently",
        ...
        final res = await toArrayAsync(concurrentAsync(2, concatAsync(a, b)));
        expect(res, equals([1, 2, 3, 4, 5, 6]));
      ...
      test("should be concatenated given 'Iterable' and 'AsyncIterable'",
        ...
        final res2 =
            await fx([1, 2, 3]).toAsync().concat(toAsync([4, 5, 6])).toArray();
