...
void main() {
  group('retry', () {
    ...
    test('should rethrow the last error when attempts run out', () async {
      ...
      await expectLater(
          ...
          throwsA(isA<StateError>()
              .having((e) => e.message, 'message', 'boom 3')));
      expect(flaky.calls, equals(3));
    });
    ...
    test('should call delay with the failure count between runs', () async {
      final delays = <int>[];
      final flaky = Flaky(2);
      await retry(3, flaky.call, delay: (failed) {
        delays.add(failed);
  ...
  group('mapRetry', () {
    test('should retry only the failing elements, in order', () async {
      ...
      final res = await toListAsync(mapRetryAsync(3, (int a) async {
    ...
    test('should propagate the error once attempts run out', () async {
      var calls = 0;
      await expectLater(
        toListAsync(mapRetryAsync(2, (int a) async {
          if (a == 2) {
            calls++;
    ...
    test('should reject a non-positive attempt count', () {
      expect(() => mapRetryAsync(0, (int a) => a, toAsync([1])),
          throwsArgumentError);
    });
