...
void main() {
  group('timeout', () {
    test('should pass values through when every pull is fast enough',
        () async {
      final res = await toListAsync(timeoutAsync(
    ...
    test('should fail the pull that exceeds the limit', () async {
      await expectLater(
        toListAsync(timeoutAsync(
            ...
            mapAsync(
                (int a) => delay(
                    Duration(milliseconds: a == 2 ? 200 : 10), a),
                toAsync([1, 2, 3])))),
    ...
    test('should keep overlapping pulls independent under concurrent',
        () async {
