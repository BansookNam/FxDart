...
void main() {
  group('mapConcurrent', () {
    group('sync source', () {
      ...
      test(
        'should keep source order even when later elements finish first',
        () async {
          Future<int> work(int n) async {
    ...
    group('async source', () {
      ...
      test('should behave exactly like map + concurrent', () async {
        ...
        final combined = await fx([
          1,
          2,
          3,
          4,
        ]).toAsync().mapConcurrent(2, work).toList();
        final split = await fx([
          1,
          2,
          3,
          4,
        ]).toAsync().map(work).concurrent(2).toList();
      ...
      test('should be able to be used mid-pipeline', () async {
        ...
        ).mapConcurrent(2, (n) async => n * 2).filter((n) => n > 2).toList();
