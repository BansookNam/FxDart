  ...
  group('ifEmpty', () {
    group('sync', () {
      ...
      test('should switch to the fallback when empty', () {
        ...
        expect(toList(ifEmpty(() => [7, 8], filter((int a) => a > 10, [1, 2]))),
            equals([7, 8]));
      });
    ...
    group('async', () {
      test('should pass a non-empty source through untouched', () async {
        var called = false;
        final res = await toListAsync(ifEmptyAsync(() {
          called = true;
          return toAsync([0]);
        }, toAsync([1, 2])));
        expect(res, equals([1, 2]));
      ...
      test('should switch to the fallback when empty', () async {
        expect(await toListAsync(ifEmptyAsync(() => toAsync([0]), asyncEmpty<int>())),
            equals([0]));
      });
      ...
      test('defaultIfEmpty should accept a future default', () async {
        expect(
            ...
            equals([0]));
        expect(await toListAsync(defaultIfEmptyAsync(0, toAsync([1, 2]))),
            equals([1, 2]));
      });
      ...
      test('should be able to be used as a chaining method in the `fx`',
        ...
        expect(
            ...
            equals([1]));
        expect(await fx([5]).toAsync().defaultIfEmpty(0).toList(), equals([5]));
      });
    });
  });
}
