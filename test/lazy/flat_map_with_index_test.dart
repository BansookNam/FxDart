  ...
  group('flatMapWithIndex', () {
    group('sync', () {
      test('flattens what the callback returns', () {
        expect(toList(flatMapWithIndex((a, i) => [i, a], [10, 20])),
            equals([0, 10, 1, 20]));
      });

      test('the index counts source elements, not emitted ones', () {
        final seen = <int>[];
        toList(flatMapWithIndex((int a, int i) {
          seen.add(i);
      ...
      test('an empty inner iterable emits nothing but advances the index', () {
        expect(
            toList(flatMapWithIndex(
                (a, i) => i.isEven ? [a] : <String>[], ['a', 'b', 'c'])),
            equals(['a', 'c']));
      });

      test('is empty for an empty source', () {
        expect(toList(flatMapWithIndex((a, i) => [a], <int>[])), equals(<int>[]));
      });
      ...
      test('is available as an fx chain method', () {
        final res = fx(['a', 'b'])
            .flatMapWithIndex((a, i) => List.filled(i + 1, a))
            .toList();
    ...
    group('async', () {
      ...
      test('the index counts source elements', () async {
        final seen = <int>[];
        await toListAsync(flatMapWithIndexAsync((int a, int i) {
          seen.add(i);
          return List.filled(a, a);
        }, toAsync([3, 2, 1])));
        expect(seen, equals([0, 1, 2]));
      ...
      test('is available as an fxAsync chain method', () async {
        final res = await fxAsync(toAsync(['a', 'b']))
            .flatMapWithIndex((a, i) => List.filled(i + 1, a))
            .toList();
        expect(res, equals(['a', 'b', 'b']));
      });
