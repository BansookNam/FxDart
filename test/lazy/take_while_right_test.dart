  ...
  group('takeWhileRight', () {
    group('sync', () {
      test('returns the trailing run in source order', () {
        expect(toList(takeWhileRight((int a) => a > 2, [1, 4, 2, 3, 4])),
            equals([3, 4]));
      });

      test('is empty when the last element already fails', () {
        expect(toList(takeWhileRight((int a) => a > 2, [3, 4, 1])),
            equals(<int>[]));
      });

      test('takes everything when every element matches', () {
        expect(toList(takeWhileRight((int a) => a > 0, [1, 2, 3])),
            equals([1, 2, 3]));
      });

      test('is empty for an empty source', () {
        expect(toList(takeWhileRight((int a) => true, <int>[])),
            equals(<int>[]));
      });

      test('an earlier matching run does not survive a later failure', () {
        expect(toList(takeWhileRight((int a) => a.isEven, [2, 2, 1, 4])),
            equals([4]));
      });
      ...
      test('the buffered path agrees with the List fast path', () {
        for (final source in [
          ...
          expect(toList(takeWhileRight((int a) => a > 2, lazily(source))),
      ...
      test('trims a trailing suffix, the usual use', () {
        expect(
            toList(takeWhileRight(
                (String c) => c == ' ', ['a', 'b', ' ', ' ', ' '])),
            equals([' ', ' ', ' ']));
      });

      test('is available as an fx chain method', () {
        expect(fx([1, 4, 2, 3, 4]).takeWhileRight((a) => a > 2).toList(),
            equals([3, 4]));
      });
