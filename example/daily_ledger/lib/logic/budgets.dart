...
Map<String, double> spentByCategory(List<Entry> entries, DateTime month) {
  ...
  return fromEntries(fx(groups.entries).map((kv) =>
...
List<BudgetStatus> budgetStatuses(
    List<Entry> entries, Map<String, double> budgets, DateTime month) {
...
Map<String, double> suggestedBudgets(List<Entry> entries, DateTime month,
    {int months = 3}) {
  final perMonthSpend = fx(range(1, months + 1))
      .map((back) => spentByCategory(
          entries, DateTime(month.year, month.month - back)))
      .toList();
  final averages = fromEntries<String, Object?>(fx(perMonthSpend)
      .flatMap((m) => m.keys)
      .uniq()
      .map((categoryId) =>
          (categoryId, fx(perMonthSpend).map((m) => m[categoryId] ?? 0.0).average())));
