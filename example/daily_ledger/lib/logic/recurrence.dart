...
List<DateTime> occurrences(RecurringRule rule, DateTime from, DateTime horizon) {
...
List<Entry> projectRule(
  ...
  final template = last(fx(existing)
      .filter((e) => e.recurringRuleId == rule.id)
      .sortBy((e) => e.date));
  if (template == null) return const [];
  return fx(occurrences(rule, from, horizon))
      .zipWithIndex()
      .map((pair) => template.copyWith(
...
List<Entry> projectAll(
  ...
  final materialized = fx(existing)
      ...
      .map((e) => '${e.recurringRuleId}@${e.date.year}-${e.date.month}-${e.date.day}')
      .toList();
  return fx(rules)
      .flatMap((rule) => projectRule(rule, existing, from, horizon))
      .filter((ghost) => !materialized.contains(
