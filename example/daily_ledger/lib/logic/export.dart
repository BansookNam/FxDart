...
const csvColumns = ['date', 'type', 'title', 'category', 'amount', 'tags', 'done'];
...
String entriesToCsv(List<Entry> entries,
    {Map<String, Category> categories = const {}}) {
  final rows = fx(entries).sortBy((e) => e.date).map((e) {
  ...
  }).map((row) => fx(csvColumns).map((c) => _escape(row[c])).toList().join(','));
