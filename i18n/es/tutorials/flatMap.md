...
description: Tutorial de expand en FxDart: transforma cada elemento en un iterable y aplana un nivel, con un playground en vivo.
...
---
  ...
  <h2>Lección</h2>
  <p>
    <code>expand</code> es <code>map</code> seguido de un nivel de
    ...
    un rango en pares. <code>expand</code> es el nombre idiomático en Dart
    (coincide con
    <a href="https://api.dart.dev/stable/dart-core/Iterable/expand.html"><code>Iterable.expand</code></a>);
    fxdart también acepta la grafía <code>flatMap</code> de FxTS: son el
    mismo operador.
  </p>
  <p>
    <strong>Desviación respecto a FxTS:</strong> en FxTS, el callback puede devolver cualquier mezcla
    de valores sueltos e iterables y el operador deduce qué
  ...
  <p>
    ...
    <code>.expand((list) =&gt; list)</code> para aplanar las
  ...
  <p>
    ...
    y deja que <code>expand</code> aplane los resultados de forma síncrona:
  </p>
  {{playground:1}}

  <h2>Pruébalo tú</h2>
  <p>Ejercicio: usa <code>expand</code> para partir cada frase en palabras,
