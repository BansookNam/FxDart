...
description: 300 ms 윈도마다 탭 하나만 통과 — 탭 스트림 위의 throttleTime vs fxdart 이벤트 레이어의 동등한 throttle 체인.
...
---
  ...
  <h2>차이가 나는 이유</h2>
  <p>
    다르지 않습니다. 스로틀링은 <em>시간 속의 도착</em>에 대한
    속도 제한 — 값이 아니라 이벤트가 언제 일어나는가의 속성 — 이고,
    두 패널 모두 요구사항을 탭 스트림 위의 연산자 하나로
  ...
  <p>
    fxdart는 push 쪽을 위해 Rx의 접근을 의도적으로 흡수해
    ...
    이 예제 같은 일상적인 시간 동사들에서는 둘이 서로 대체
