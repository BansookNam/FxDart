...
description: FxDart distinctBy 튜토리얼: 계산된 키를 기준으로 중복을 제거하는 방법을 라이브 플레이그라운드와 함께 익힙니다.
...
---
  ...
  <h2>강의</h2>
  <p>
    <code>distinctBy</code>는 <code>uniq</code>의 일반화된 형태입니다. 원소
    전체를 비교하는 대신 각 원소를 <code>f</code>에 통과시켜, 서로 다른 키마다
    그 키를 처음 만들어 낸 원소만 남깁니다. <code>distinctBy</code>가
    Dart다운 이름이고, fxdart는 FxTS식 표기인 <code>uniqBy</code>도 함께
  ...
  <h2>직접 해 보기</h2>
  <p>연습: <code>distinctBy</code>로 각 <code>'type'</code>의 첫 이벤트만
    남겨 보세요.</p>
  {{playground:2}}
