...
description: FxDart indexWhere 튜토리얼: 지연 평가와 단락 평가로 첫 일치 원소의 위치를 찾고, 없으면 -1을 받는 방법을 익힙니다.
...
---
  ...
  <h2>강의</h2>
  <p>
    <code>indexWhere</code>는 Dart다운 이름이고(<code>List.indexWhere</code>와
    짝을 이룹니다), fxdart는 FxTS식 표기인 <code>findIndex</code>도 함께
    받습니다 — 같은 연산자입니다. <code>find</code>의 형제로서 지연 평가와
    단락 평가로 훑는 방식은 똑같지만, 일치한 값 자체가 아니라 그것이
  ...
  <h2>데모 2 · 단락 평가와 비동기</h2>
  {{playground:1}}

  <h2>직접 해 보기</h2>
  <p>연습: <code>indexWhere</code>로 큐에서 bob의 위치를 찾고, 없으면 <code>-1</code>을 받아 보세요.</p>
  {{playground:2}}
