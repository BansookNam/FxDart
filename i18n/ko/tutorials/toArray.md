...
description: FxDart toArray 튜토리얼: 지연 평가 체인을 List로 실체화하는 종결 연산자를 동기와 비동기 양쪽에서 다룹니다.
...
---
  ...
  <h2>강의</h2>
  <p>
    <code>toArray</code>는 가장 많이 쓰이는 <strong>종결 연산자</strong>입니다.
    그 시점까지 계획에 불과했던 체인을 실제로 실행시키는 것이 바로 이 함수입니다.
    <code>.toArray()</code>를 호출하면 이터러블에 남아 있는 모든 값을 순서대로
    끌어당겨 진짜 <code>List&lt;T&gt;</code>에 담습니다. 상류의 모든 것 —
    <code>.map()</code>, <code>.filter()</code>, <code>.take()</code> — 은
    <code>toArray</code>가 값을 요청했기 때문에 비로소 실행됩니다.
  </p>
  <p>
    뒤집어 말하면 <code>toArray</code>는 무한하거나 크기가 정해지지 않은 소스에
    ...
    한정한 다음, 그 결과에 <code>toArray</code>를 호출하세요.
  </p>
  <p>
    비동기 버전인 <code>toArrayAsync</code>(또는 <code>FxAsync</code> 체인에서의
    <code>.toArray()</code>)는 원소를 끌어당길 때마다 await하며
  ...
  <p>
    <code>FxAsync</code> 체인에서의 <code>.toArray()</code>는 모든 원소를
  ...
  <div class="callout">
    ...
    <a href="fx.html"><code>fx</code></a> — toArray가 종결시키는 그 체인 ·
