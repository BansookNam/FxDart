...
description: FxDart where 튜토리얼: 술어가 통과시킨 원소만 남기는 방법을 동기와 비동기 양쪽에서 라이브 플레이그라운드와 함께 익힙니다.
...
---
  ...
  <h2>강의</h2>
  <p>
    <code>where</code>는 <code>map</code>과 더불어 가장 기본이 되는
    변환 연산자입니다. 술어가 <code>true</code>를 반환한 원소만 남기고
    나머지는 버리며, 이 모든 과정은 지연 평가됩니다. 종결 연산자가 값을
    끌어당기기 전까지는 아무것도 실행되지 않습니다. <code>where</code>가
    Dart다운 이름이고, fxdart는 FxTS식 표기인 <code>filter</code>도 함께
    받습니다 — 같은 연산자입니다.
  </p>
  <p>
    지연 평가되므로 <code>where</code>를 <code>take</code>와 조합하면
  ...
  <h2>직접 해 보기</h2>
  <p>연습: <code>where</code>로 18세 이상만 남겨 보세요.</p>
  {{playground:2}}
