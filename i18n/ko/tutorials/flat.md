...
description: FxDart flattened 튜토리얼: 중첩된 이터러블을 원하는 깊이만큼 평탄화하는 방법을 라이브 플레이그라운드와 함께 익힙니다.
...
---
  ...
  <h2>강의</h2>
  <p>
    <code>flattened</code>는 중첩된 이터러블 구조를 훑으면서 안쪽 원소를 바깥
    ...
    그래서 <code>flattened(['ab', ['cd']])</code>는 <code>'ab'</code>를 글자
    단위로 쪼개지 않고 그대로 둡니다. <code>flattened</code>가 Dart다운
    이름이고, fxdart는 FxTS식 표기인 <code>flat</code>도 함께 받습니다 —
    같은 연산자입니다.
  </p>
  <p>
  ...
  <p>
    ...
    <code>.flattened()</code>로 이어 붙이면 됩니다.
  </p>
  ...
  <h2>직접 해 보기</h2>
  <p>연습: <code>flattened()</code>로 <code>scoreGroups</code>를 한 단계
    평탄화해 보세요.</p>
  {{playground:2}}
